...
def build_hitter_pitcher_pa_game_agg() -> None:
    logger.info("Building mlb.dbo.fact_hitter_pitcher_pa_game_agg")

    sql = """
IF OBJECT_ID('mlb.dbo.fact_hitter_pitcher_pa_game_agg', 'U') IS NOT NULL
    DROP TABLE mlb.dbo.fact_hitter_pitcher_pa_game_agg;
...
INTO mlb.dbo.fact_hitter_pitcher_pa_game_agg
FROM base
GROUP BY
    gamePk,
    game_date,
    ...
    execute_sql(sql)
    logger.info("Finished building mlb.dbo.fact_hitter_pitcher_pa_game_agg")


def build_hitter_pitcher_pa_rolling_features() -> None:
    logger.info("Building mlb.dbo.fact_hitter_pitcher_pa_rolling_features")

    sql = """
IF OBJECT_ID('mlb.dbo.fact_hitter_pitcher_pa_rolling_features', 'U') IS NOT NULL
    DROP TABLE mlb.dbo.fact_hitter_pitcher_pa_rolling_features;
...
SELECT *
INTO mlb.dbo.fact_hitter_pitcher_pa_rolling_features
FROM rolling;
    """

    execute_sql(sql)
    logger.info("Finished building mlb.dbo.fact_hitter_pitcher_pa_rolling_features")


def run_all_hitter_pitcher_pa_features() -> None:
    logger.info("Starting hitter vs pitcher plate appearance feature pipeline")
    build_hitter_pitcher_pa_game_agg()
    build_hitter_pitcher_pa_rolling_features()
    logger.info("Finished hitter vs pitcher plate appearance feature pipeline")
