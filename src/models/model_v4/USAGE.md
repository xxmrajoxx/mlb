...
6. [What the Model Outputs](#6-what-the-model-outputs)
7. [What You Need to Update Manually](#7-what-you-need-to-update-manually)
...
- The four SQL Server tables (covered in section 6)

---

## 4. Initial Setup (one-time)

You've already done this, but for reference / future you:
...
### Step 3: Confirm source table exists
In SSMS:
```sql
SELECT TOP 10 * FROM mlb.dbo.fact_hitter_pitcher_matchup_model_featuresv2;
```
...
From your project's `model_v4` folder:
...
The pipeline executes in this order. Approximate runtime: **8–15 minutes** depending on data volume.

| Stage | What it does | Time |
|---|---|---|
| 1. Load matchup data | Pulls all rows from the source table | 30 sec – 2 min |
| 2. Split by season | Train: 2023–24, Validation: 2025, Test: 2026 | instant |
| 3. Feature prep | Selects features, encodes categoricals | 30 sec |
| 4. Train XGBoost | Gradient boosted trees with early stopping | 1–3 min |
| 5. Train LightGBM | Second tree model for ensemble diversity | 1–2 min |
| 6. Train Logistic Regression | Linear baseline | 30 sec – 1 min |
| 7. Calibrate (isotonic) | Maps raw probabilities to true probabilities | a few seconds |
| 8. Build ensemble | Weighted average: 50% XGB, 40% LGBM, 10% LogReg | instant |
| 9. Evaluate | AUC, Brier, log loss, MAE, etc. | instant |
| 10. Aggregate to game level | Sum per-PA P(K) into pitcher-game totals | 1–2 min |
| 11. Save artefacts | Writes models to `./artifacts/` | a few seconds |
| 12. Write to SQL Server | Loads four output tables | 1–3 min |

### When to re-run

- **Every 1–2 weeks** during the season — picks up new game data
- **After roster changes** at the trade deadline or post-injury
- **When metrics start drifting** (see section 9)
...
3. Glance at performance (see section 10).
...
## 9. Metrics Reference: What Good and Bad Look Like
...
**Note on the current model:** the bias on the test set is around +0.3, which is just outside the "Good" range. This is patched in `compute_ev.sql` via per-bucket bias correction (see section 10) so it doesn't affect bet recommendations, but the underlying number is what you'd see in `fact_model_evaluation_metrics`.
...
| `predicted_strikeouts` looks far too low (< 2 for starters) | `hitter_plate_appearances` not populating from source view | Run the diagnostic queries in section 9 — likely a source data issue |
...
- Check the metrics dashboard (section 9) for drift
