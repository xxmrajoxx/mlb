...
def fetch_player_game_logs(start_date: str, end_date:str, season: int = 2026)->pd.DataFrame:
    ...
    completed_gamepks = set(completed_games_df["gamePk"].dropna().astype(int).tolist())
    ...
    for _, player_row in player_df.iterrows():
        ...
        splits = stats_list[0].get("splits",[])             # next step down
        ...
        for s in splits:
            stat = s.get("stat", {})                            # print(hitting["stats"][0]["splits"][0][stat]
            game = s.get("game", {})                            # print(hitting["stats"][0]["splits"][0][game]
